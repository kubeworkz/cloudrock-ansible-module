...
import cloudrock_marketplace


@mock.patch('cloudrock_marketplace.cloudrock_client_from_module')
@mock.patch('cloudrock_marketplace.AnsibleModule')
class OrderItemCreateTest(unittest.TestCase):
    ...
    def test_create_marketplace_order_item(
        self, mock_ansible_module, mock_cloudrock_client
    ):
        client = mock.Mock()
        client.create_marketplace_order.return_value = {
            'items': [{'uuid': 'order_item_uuid'}]
        }
        _, has_changed = cloudrock_marketplace.send_request_to_cloudrock(client, self.module)
